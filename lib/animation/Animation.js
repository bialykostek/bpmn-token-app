...
Animation.prototype.createAnimation = function(connection, scope, done = noop) {
  ...
  //var lx = parseInt(document.querySelectorAll('[data-element-id="'+connection.id+'"]')[0].parentElement.querySelector('rect').attributes.x.value);
  //var ly = parseInt(document.querySelectorAll('[data-element-id="'+connection.id+'"]')[0].parentElement.querySelector('rect').attributes.y.value);
  console.log(connection)
  var lx = connection.waypoints[0].x;
  var ly = connection.waypoints[0].y;


  console.log(connection)
  if(connection.id != "SequenceFlow_0rz4mzx" && connection.id != "SequenceFlow_1ge46mh"){
    if(document.querySelectorAll('[data-element-id="'+connection.id+'"]')[0].parentElement.querySelectorAll('.token-counter-mark').length == 0){

      document.querySelectorAll('[data-element-id="'+connection.id+'"]')[0].parentElement.innerHTML += `<g class="bts-token token-counter-mark" transform="translate(`+lx+`, `+(ly-30)+`)">
        <circle class="bts-circle" r="10" cx="10" cy="10" fill="#1f38f4"></circle>
        <text class="bts-text token-counter-value" transform="translate(10, 14)" text-anchor="middle" fill="#fff">1</text>
      </g>`


    }else{
      document.querySelectorAll('[data-element-id="'+connection.id+'"]')[0].parentElement.querySelectorAll('.token-counter-value')[0].innerHTML = parseInt(document.querySelectorAll('[data-element-id="'+connection.id+'"]')[0].parentElement.querySelectorAll('.token-counter-value')[0].innerHTML) + 1;
    }
  }
